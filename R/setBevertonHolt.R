...
#' `erepro`, `R_max` or `reproduction_level` you pick the desired reproduction
...
#' of the parameters `R_max` and `erepro`. A higher `R_max` coupled with a lower
#' `erepro` can give the same value as a lower `R_max` coupled with a higher
#' `erepro`.
#'
...
#' curves going through that point. This parameter can be `erepro`, `R_max` or
#' the `reproduction_level`.
#'
...
#'   See details.
#' @param R_factor `r lifecycle::badge("deprecated")` Use `reproduction_level`
#'   instead, with `R_factor = 1 / reproduction_level`.
#'
