...
#' Match abundances to observations
#' 
#' `r lifecycle::badge("experimental")`
#' The function adjusts the abundances of the species in the model so that their abundances
...
#' to steady state, after which of course the abundances will no longer match
...
#' `abundance_observed` column to your model which gives the observed abundance in
#' grams.  For species for which you have no observed abundance, you should set
#' the value in the `abundance_observed` column to 0 or NA.
#'
...
#'   abundances will be matched. A vector of species names, or a numeric vector
...
#' params <- calibrateAbundance(params)
#' params <- matchAbundances(params)
#' plotAbundanceObservedVsModel(params)
matchAbundances <- function(params, species = NULL) {
    if (!("abundance_observed" %in% names(params@species_params))) {
        return(params)
    }
    species <- valid_species_arg(params, species = species, 
                                 return.logical = TRUE) &
        !is.na(params@species_params$abundance_observed) &
        params@species_params$abundance_observed > 0
    for (sp in (1:nrow(params@species_params))[species]) {
        cutoff <- params@species_params$abundance_cutoff[[sp]]
        if (is.null(cutoff) || is.na(cutoff)) {
            cutoff <- 0
        }
        total <- sum((params@initial_n[sp, ] * params@w * params@dw)
                     [params@w >= cutoff])
        factor <- params@species_params$abundance_observed[[sp]] / total
