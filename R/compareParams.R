...
compareParams <- function(params1, params2) {
    ...
    sp_diff1 <- setdiff(sp_names2, sp_names1)
    if (length(sp_diff1) > 0) {
        print(paste("params1 has the following additional species parameters:",
                    toString(sp_diff1)))
    }
    ...
    if (!isTRUE(sp_eq)) {
        print(paste("The following species parameters differ:",
              toString(sp_eq)))
    ...
    if (!isTRUE(res_eq)) {
        print(paste("The following resource parameters differ:",
                    toString(res_eq)))
    }

    # size grid ----
    if (length(params1@w) != length(params2@w)) {
        print("The community size bins differ.")
        return(invisible())
    }
    if (length(params1@w_full) != length(params2@w_full)) {
        print("The resource size bins differ.")
        return(invisible())
    }
    ...
    for (sl in slotNames(params1)) {
        ...
        if (!isTRUE(eq)) {
            print(paste("The", sl, "slots do not agree:",
                        toString(eq)))
        }
    }
    invisible()
