...
to their maximum size. Let us explore this analogy briefly.
...
The flow of traffic on a highway is dependent on the traffic density, which in 
turn is dependent on changes in the traffic velocity.
...
`lambda = -2.05`. Thus the number density is proportional to $w^{-2.05}$.
...
would be nice if we could get at the actual numbers. The `steady()` function
has stored those numbers as the initial condition in the params object and we
can access them with the `initialN()` function. Let us assign this to a
variable `n`:
```{r}
n <- initialN(params)
```
...
number by the width of the size class. Those widths can be obtained with the
`dw()` function. So the number of fish in the 151st
size class is
```{r}
(n * dw(params))[1, 151]
```

You may be surprised ny the small number if you interpret it as the number of
...
a discussion of this issue of working with numbers per area,
...
tutorial and do exercise 2 before reading on.
...
class. To obtain these weights, we use the function `w()` that returns the
weights at the start of each size class. Of course using these will lead to a
discretisation error because not all fish in the size class have the same 
weight, but with the small size classes that we use
...
straight line. This is due to the allometric scaling of the physiological rates,
...
First of all, the metabolic rate, i.e.,
...
instantaneous per-capita growth rate, measured in grams per year. 
...
251 observations correspond to the 251 size classes. The 3 variables have 
names
```{r}
...
growth_rate. This data frame we can pass to `ggplot()`.
```{r}
...
variable and specified that both axes should be on a logarithmic scale.
...
power laws with exponents $p=3/4$ and $p-1=-1/4$ respectively. By
solving a differential equation we can derive that the juvenile spectrum also
follows a power law:
$$N(w) = N_0\ w^{-\mu_0/g_0 - p}$$

I won't do the maths here with you (and you probably don't want me to anyway), 
but we can check this claim numerically. Let's look at the spectrum up to 
...
[mizer documentation](https://sizespectrum.org/mizer/reference/setReproduction.html#investment). 
...
curve in the same plot? First we 
add an extra column to each dataframe describing it
```{r}
psi$type = "original"
...
Next let us look at how the steady state spectrum has changed. We first need to
...
* what sizes of prey the fish is able to filter out of the water, which 
...
As usual, we first create a copy of the parameter object, then we make the
change in that copy.
```{r message=FALSE}
params_pk <- params
...
to model from first principles, although people have tried to argue in terms
of swimming speeds of fish. We will simply assume that this search volume rate
is also an allometric rate. Let $\gamma(w)$ denote this rate for a predator of
size $w$. The we assume that
$$\gamma(w) = \gamma_0\ w^q$$
for some exponent $q$. 
...
that $N(w) = N_0\ w^{-\lambda}$. A bit of maths
then says that
$$q = 2 - \lambda + n$$
...
## Feeding level
A predator will have a maximum intake rate. It will simply not be able to 
...
don't have to introduce new parameters for death from predation.
...
have not included in our model, like sea birds or mammals, or it could be death
from old age (senescent death) or disease. Mizer allows setting of
background death with `setExtMort()`.
...
We also need to specify the fishing effort and can then plot the resulting
fishing mortality.
```{r}
...
After having turned on fishing, we need to find the new steady state with
`steady()` and can then plot it.
```{r message=FALSE}
